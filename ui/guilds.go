...
func (gl *GuildsList) onSelected(idx int, mainText string, secondaryText string, shortcut rune) {
	...
	if mainText == "Direct Messages" {
		...
		for _, c := range cs {
			if !discord.HasPermission(gl.app.Session.State, c.ID, astatine.PermissionViewChannel) {
				continue
			}
		...
		for _, c := range cs {
			if c.Type == astatine.ChannelTypeGuildCategory {
				...
				for _, nestedChannel := range cs {
					if !discord.HasPermission(gl.app.Session.State, nestedChannel.ID, astatine.PermissionViewChannel) {
						continue
					}
